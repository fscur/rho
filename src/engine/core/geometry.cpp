...
namespace rho
{
    ...
    geometry* geometry::create(guid guid, vector<vertex> vertices, vector<uint> indices)
    {
        ...
        auto geometry = new rho::geometry(guid);
        ...
        geometry->aabb = new rho::aabb(calcAabb(vertices));
    ...
    aabb geometry::calcAabb(vector<vertex>& vertices)
    {
        ...
        return rho::aabb(vec3(minX, minY, minZ), vec3(maxX, maxY, maxZ));
    }
    ...
    {
        auto octree = rho::octree(rho::aabb(glm::vec3(-0.7, -0.7, -0.7), glm::vec3(0.7, 0.7, 0.7)), 5, 50);
