...
namespace rho
{
    ...
    public:
        ...
        static double easeOutElastic(double t)
        {
            ...
            {
                s = p / (2.0 * rho::PI) * std::asin(1.0 / a);
            }

            return a * std::pow(2.0, -10.0 * t) * std::sin((t - s) * (2.0 * rho::PI) / p) + 1.0;
        }
        ...
        {
            return static_cast<double>(glm::sin(t * rho::PI));
        }

        static double easeRubberBack(double t)
        {
            return static_cast<double>(glm::sin(glm::pow(t, 1.0 / 2.0) * rho::PI));
        }
    };
}