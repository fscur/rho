...
namespace rho
{
    ...
    //            {
    //                //case 0:
    //                //    objectNode->addComponent(new rho::model());
    //                //    break;
    //            case 1:
    ...
    //
    //                objectNode->addComponent(new rho::mesh(geometry, material));
    //
    //                auto aabb = geometry->aabb;
    //                objectNode->addComponent(new rho::boxCollider(aabb->center, vec3(aabb->width, aabb->height, aabb->depth)));
    //                objectNode->addComponent(new rho::animator());
    ...
    //        auto guid = rho::guid(geometryGuidBytes);
    ...
    //        if (foundAabb)
    //        {
    //            auto boxCollider = new rho::boxCollider(rootAabb.center, rootAabb.getSize());
    //            boxCollider->disable();
    //            rootNode->addComponent(boxCollider);
    //        }
    //
    //        rootNode->addComponent(new rho::mouseInteractionComponent());
    //        // TODO: do not add this collider after the nodes have obb information
    //        auto rootBoxCollider = new rho::boxCollider(rootAabb.center, vec3(rootAabb.width, rootAabb.height, rootAabb.depth));
    //        rootBoxCollider->disable();
    //        rootNode->addComponent(rootBoxCollider);
    //
    //        auto guid = convertToGuid(document["Guid"].GetString());
    //        auto model = new rho::model(guid, rootNode);
    //
    //        return model;
    //    }
    //
    //    vector<node*> importer::loadrhoFile(
    //        const string& fileName,
    //        const entityRepository<model>* modelsRepository)
    //    {
    //        const string fileContents = fileReader::readFile(fileName);
    //        Document* rhoJsonDoc = getJsonDocumentFromrhoFile(fileContents);
    //        //auto camera = loadCamera(rhoJsonDoc);
    //        return loadNodes(rhoJsonDoc, modelsRepository);
    //    }
    //
    //    Document* importer::getJsonDocumentFromrhoFile(const string& rhoFileContents)
    //    {
    //        Document* doc = new Document();
    //        doc->Parse(rhoFileContents.c_str());
    //        return doc;
    //    }
    //
    //    std::vector<node*> importer::loadNodes(
    //        const Document* rhoJsonDoc,
    //        const entityRepository<model>* modelsRepository)
    //    {
    //        std::vector<node*> loadedNodes;
    //        if (rhoJsonDoc->IsObject() && rhoJsonDoc->HasMember("scene") && rhoJsonDoc->HasMember("nodes"))
    //        {
    //            auto& nodes = (*rhoJsonDoc)["nodes"];
    //            if (nodes.IsObject())
    //            {
    ...
    //    }
    //
    //    camera* importer::loadCamera(const Document* rhoJsonDocument)
    //    {
    //        auto& cameraJsonValue = (*rhoJsonDocument)["camera"];
    ...
    //
    //        auto transform = new rho::transform();
