...
namespace rho
{
        ...
        _hinting(false)
    {
        if (font::FreeTypeLibrary == nullptr)
            throw rho::exception("FreeType not initialized. Inititialized Font Manager? ;)");
    ...
    {
        auto glyrhondex = FT_Get_Char_Index(_fontFace, glyphChar);
        return getGlyph(glyrhondex);
    }

    glyph* font::getGlyph(const uint& glyrhondex)
    {
        if (_glyphCache.find(glyrhondex) != _glyphCache.end())
            return _glyphCache[glyrhondex];

        FT_GlyphSlot glyphSlot = _fontFace->glyph;

        FT_Library_SetLcdFilterWeights(font::FreeTypeLibrary, _lcdWeights);

        FT_Load_Glyph(_fontFace, glyrhondex, _loadGlyphFlags);
        ...
        auto glyph = new rho::glyph(glyrhomage);
        glyph->index = glyrhondex;
