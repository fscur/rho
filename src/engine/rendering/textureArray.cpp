...
namespace rho
{
    textureArray::textureArray(
        ...
        _texturesAddresses(map<const texture*, rho::textureAddress>())
    {
        ...
        textureAddress& textureAddress)
    {
        ...
        auto atlasItem = new rho::atlasItem(rect, texture);

        rho::atlasNode* atlasNode = nullptr;
        ...
        if (atlasNode)
        {
            ...
            textureAddress = rho::textureAddress();
            textureAddress.arrayId = _id;
            textureAddress.unit = _unit;
            textureAddress.page = page;
    ...
    float textureArray::getEmptyPage()
    {
        for (auto i = 0u; i < _maxPages; ++i)
        {
    ...
    float textureArray::getAvailablePage()
    {
        for (auto i = 0u; i < _maxPages; ++i)
        {
            auto key = static_cast<float>(i);
    ...
    void textureArray::onLoadSubData(
        const rectangle<GLint>& rect,
        float page,
        const void* const data)
    {
    ...
    bool textureArray::add(
        const texture* const texture,
        textureAddress& textureAddress)
    {
        if (rho::contains(_textures, texture))
        {
    ...
    void textureArray::remove(const texture * texture)
    {
        auto address = _texturesAddresses[texture];
        
        rho::removeIfContains(_textures, texture);
