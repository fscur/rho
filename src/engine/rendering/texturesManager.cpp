...
namespace rho
{
    ...
    void texturesManager::removeTexture(texture* texture)
    {
        ...
        if (textureArray->isEmpty())
        {
            if (_isBindless)
            {
                ...
                rho::removeIfContains(textureArraysHandles, handle);
            }
    ...
    void texturesManager::removeCubeMap(cubeMap* cubeMap)
    {
        ...
        if (cubeMapArray->isEmpty())
        {
            if (_isBindless)
            {
                ...
                rho::removeIfContains(cubeMapArraysHandles, handle);
            }
    ...
    {   
        ...
        auto textureAddress = rho::textureAddress();
        ...
        if (it != _textureArrays.end())
        {
            ...
            if (added)
            {
                index = static_cast<int>(rho::indexOf(textureArraysUnits, textureAddress.unit));
    ...
    textureAddress texturesManager::addCubeMap(const cubeMap* const cubeMap)
    {
        ...
        auto cubeMapAddress = rho::textureAddress();
        ...
        if (it != _cubeMapArrays.end())
        {
            ...
            if (added)
            {
                auto index = static_cast<int>(rho::indexOf(cubeMapArraysUnits, cubeMapAddress.unit));
    ...
    {
        auto cubeMapArray = new rho::cubeMapArray(size, layout);
    ...
    texture* texturesManager::getTextureFromImage(rho::image* image, bool generateMipmaps)
    {
        if (!image)
            throw argumentException("image cant be nullptr.");

        rho::texture* texture = nullptr;
    ...
    {
        return rho::contains(_textures, texture);
    }

    bool texturesManager::contains(const cubeMap* const cubeMap)
    {
        return rho::contains(_cubeMaps, cubeMap);
    }
    ...
    textureArray* texturesManager::createTextureArray(sizeui size, textureLayout layout)
    {
        ...
        if (_isSparse && _isBindless)
            ...
            textureArray = new rho::textureArray(size, layout);
    ...
    {
        rho::removeIfContains(_textureArrays[textureArray->getLayout()], textureArray);
    }

    void texturesManager::removeCubeMapArray(cubeMapArray* cubeMapArray)
    {
        rho::removeIfContains(_cubeMapArrays[cubeMapArray->getLayout()], cubeMapArray);
    }
