...
namespace rho
{
    ...
    model* modelDataService::getModelByIndex(int index)
    {
        ...
        return new rho::model(model.guid, node);
    }

    model* modelDataService::getModelById(const rho::guid& id)
    {
        //implement cache
    ...
    {
        node->addComponent(new rho::mouseInteractionComponent());
    }

    node* modelDataService::assembleNode(nodeDTO nodeDTO)
    {
        auto node = new rho::node();
