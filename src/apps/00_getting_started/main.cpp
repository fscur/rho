...
{
    setlocale(LC_ALL, "");
    bindtextdomain("getting_started", "./locale");
    textdomain("getting_started");
}

void initializeDiagnostics(const char* executableFileName)
{
    rho::platformProcLoader::load(executableFileName);
}
...
int main(int argc, char* args[])
{
    ...
    try
    {
        ...
        auto screen = getting_started::screen(L"00_getting_started", resolution(1600, 900));

        application.run(&screen);
    }
    catch (const rho::invalidInitializationException& exception)
    {
